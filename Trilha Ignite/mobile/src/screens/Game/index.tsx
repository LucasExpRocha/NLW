...
export function Game() {
  ...
  function getDuoPlayer(adsId: string) {
    fetch(`http://26.180.131.64:3636/ads/${adsId}/discord`)
    .then(response => response.json())
    .then(data => setDiscordMatch(data.discord))
  }

  useEffect(() => {
    fetch(`http://26.180.131.64:3636/games/${game.id}/ads`)
    .then(response => response.json())
    ...
    <SafeAreaView style={styles.container}>
      ...
      <FlatList
        ...
        renderItem={({item}) => (
          <DuoCard 
            data={item}
            onConnect={() => getDuoPlayer(item.id)}
          />
        )}
        horizontal
